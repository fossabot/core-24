...
export function Videos(): ReactElement {
  ...
  const onLanguageChange = (_e, option: { id: React.SetStateAction<string>; } | null):void => {
    if (option !== null) {
  ...
  const onTagChange = (_e, option: { id: React.SetStateAction<string>; } | null):void => {
    console.log(option)
    if (option !== null) {
      setTagId(option.id)
    } else {
  ...
  return (
    <>
      <Grid container spacing={4} display="flex" alignItems="baseline" sx={{my: 4}}>
        <Grid item>        
          <LanguageSearch
